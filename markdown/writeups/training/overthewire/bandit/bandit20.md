...
So, we know that echo is probably the best way to get the message to `nc` using a pipe, and since we know the passwords are going to be compared then we should use the `-n` operator to remove any trailing newline character so only the exact passweords are compared
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM3MzIxNDc5LC00NTU5MTk3MzIsMTY1Nj
