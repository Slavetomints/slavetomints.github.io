...
```
Huh, that's odd. What happened is that `-` is used for a few different things in UNIX/UNIX-like systems, most commonly to specify flags for a command such as `ls -sa`. Now whats happening is the terminal is waiting for the "rest" of the command as it doesn't think that we are quite done yet.
